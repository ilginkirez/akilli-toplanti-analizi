...
export async function createSession(sessionId?: string, participants?: string[]): Promise<SessionInfo> {
...
export async function uploadParticipantRecording(payload: {
  ...
  form.set('file', payload.file, payload.filename || `participant-${payload.participant_id}.webm`);
