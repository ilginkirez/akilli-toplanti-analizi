...
import { Mic, MicOff, Pin, PinOff, User, VideoOff } from 'lucide-react';
...
function VideoParticipantComponent({
  ...
  const hasLiveVideoTrack = Boolean(videoTrack && videoTrack.mediaStreamTrack.readyState === 'live');
  const hasLiveAudioTrack = Boolean(audioTrack && audioTrack.mediaStreamTrack.readyState === 'live');
  ...
  return (
    <div className="relative isolate h-full w-full overflow-hidden rounded-xl bg-black ring-1 ring-white/10">
      ...
      {showVideo ? (
        <video
          ...
          className={`h-full w-full transform-gpu ${
            isLarge ? 'bg-black object-contain' : 'object-cover'
          } ${isLocal ? '-scale-x-100' : ''}`}
      ...
      {showSpeakingState && (
        <div className="pointer-events-none absolute left-2 top-2 z-20 rounded-full border border-emerald-300/80 bg-emerald-400 px-3 py-1 text-[10px] font-black tracking-[0.22em] text-emerald-950 shadow-[0_12px_26px_rgba(34,197,94,0.38)]">
          KONUSUYOR
        </div>
      )}
      ...
      {showPinAction && onPinToggle && (
        <button
          ...
          className={`absolute left-2 z-10 flex items-center gap-1 rounded-full border px-2 py-1 text-[11px] font-medium ${
            isPinned
          ...
          } ${showSpeakingState ? 'top-10' : 'top-2'}`}
            ...
            {isMuted ? (
              ...
              <div
                className={`flex h-6 w-6 items-center justify-center rounded-full ${
                  showSpeakingState
                    ? 'bg-emerald-500/90 shadow-[0_0_18px_rgba(34,197,94,0.35)]'
                    : 'bg-white/10'
                }`}
              >
      ...
      {isLocal && (
        <div className="absolute right-2 top-2 rounded-md bg-indigo-500/80 px-2 py-0.5 text-[10px] font-medium text-white">
          SEN
        </div>
      )}
    </div>
